		...
		for file in files:
			if file.lower().endswith('.jpg') or file.lower().endswith('.png'):
				...
				if file.lower().endswith('eastm.png') or file.lower().endswith('east.png') or file.lower().endswith('westm.png') or file.lower().endswith('west.png'):
					...
					rightAddCut = int(height * 0.0635);
					leftAddCut = int(height * 0.0865);
					topAddCut = int(height * 0.031);
					bottomAddCut = int(height * 0.119);
					needProcess = True;
				elif file.lower().endswith('southm.png') or file.lower().endswith('south.png'):
					image = Image.open(file_path)
					width, height = image.size
					rightAddCut = int(height * 0.075);
					leftAddCut = int(height * 0.075);
					topAddCut = int(height * 0.024);
					bottomAddCut = int(height * 0.126);
					needProcess = True;
				elif file.lower().endswith('northm.png') or file.lower().endswith('north.png'):
					image = Image.open(file_path)
					width, height = image.size
					rightAddCut = int(height * 0.075);
					leftAddCut = int(height * 0.075);
					topAddCut = int(height * 0.015);
					bottomAddCut = int(height * 0.135);
					needProcess = True;
